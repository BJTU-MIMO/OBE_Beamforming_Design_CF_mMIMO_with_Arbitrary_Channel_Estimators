function [SE] = functionComputeSE_Distributed_Analytical(A,W,G_LoS_eff,R,Phi,tau_c,tau_p,Pset,N,K,M,p)
%%=============================================================
%The file is used to compute the uplink achievable SE by analytical results in the paper:
%
    ...
    for m = 1:M
        ...
        for l = 1:K
            ...
            mu_mkl(k) = mu_mkl(k) + p(l)*trace(W(:,:,m,k)'*Gbar_mkl(:,:,m,l,l)*W(:,:,m,k)*Gbar_mkl(:,:,m,k,k)) + p(l)*trace(W(:,:,m,k)'*R(:,:,m,l)*W(:,:,m,k)*Gbar_mkl(:,:,m,k,k))...
                + p(l)*trace(W(:,:,m,k)'*Gbar_mkl(:,:,m,l,l)*W(:,:,m,k)*tau_p*A(:,:,m,k)*Phi(:,:,m,k)*A(:,:,m,k)') + p(l)*trace(W(:,:,m,k)'*R(:,:,m,l)*W(:,:,m,k)*tau_p*A(:,:,m,k)*Phi(:,:,m,k)*A(:,:,m,k)');
            ...
            if any(l == Pset(:,k))
                ...
                mu_mkl(k) = mu_mkl(k) + p(l)*sqrt(p(l))*tau_p*trace(W(:,:,m,k)'*Gbar_mkl(:,:,m,l,k))*trace(W(:,:,m,k)*Phi_cross_mlk(:,:,m,l,k)') + p(l)*sqrt(p(l))*tau_p*trace(W(:,:,m,k)'*Phi_cross_mlk(:,:,m,l,k))*trace(W(:,:,m,k)*Gbar_mkl(:,:,m,k,l))...
                    + p(l)*p(l)*tau_p^2*abs(trace(W(:,:,m,k)'*Phi_cross_mlk(:,:,m,l,k)))^2;
